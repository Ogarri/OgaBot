...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('history')
        .setDescription('Affiche l\'historique de vos 20 dernières parties.'),
        ...
        try {
            ...
            if (lastMatches.length === 0) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setDescription('Aucun match trouvé dans les 20 dernières parties.');
            ...
            for (let i = 0; i < lastMatches.length; i += matchesPerEmbed) {
                const embedMatches = lastMatches.slice(i, i + matchesPerEmbed);
                const pageNum = Math.floor(i / matchesPerEmbed) + 1;
                const totalPages = Math.ceil(lastMatches.length / matchesPerEmbed);
                ...
                if (totalPages > 1) {
                    embed.setDescription(`Page ${pageNum}/${totalPages} • ${lastMatches.length} matchs total`);
                } else {
                    embed.setDescription(`${lastMatches.length} derniers matchs`);
                }
