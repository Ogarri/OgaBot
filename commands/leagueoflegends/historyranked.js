...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('historyranked')
        .setDescription('Affiche vos matchs classés 5v5 Solo parmi les 20 dernières parties.'),
    async execute(interaction) {
        try {
            ...
            // Filtrer pour ne garder que les matchs classés Solo (queueId 420)
            const rankedSoloMatches = lastMatches.filter(match => match.info.queueId === 420);
            
            if (rankedSoloMatches.length === 0) {
                const errorEmbed = new EmbedBuilder()
                    .setColor('#ff6600')
                    .setTitle('⚠️ Aucun match classé trouvé')
                    .setDescription('Il n\'y a pas de matchs classés dans les 20 derniers matchs.');
            ...
            for (let i = 0; i < rankedSoloMatches.length; i += matchesPerEmbed) {
                const embedMatches = rankedSoloMatches.slice(i, i + matchesPerEmbed);
                const pageNum = Math.floor(i / matchesPerEmbed) + 1;
                const totalPages = Math.ceil(rankedSoloMatches.length / matchesPerEmbed);
                
                const embed = new EmbedBuilder()
                    .setColor('#0099ff')
                    .setTitle(`📊 Historique Classé de ${account.gameName}#${account.tagLine}`);
                
                if (totalPages > 1) {
                    embed.setDescription(`Page ${pageNum}/${totalPages} • ${rankedSoloMatches.length} matchs classés`);
                } else {
                    embed.setDescription(`${rankedSoloMatches.length} matchs classés`);
                }
